...
import { LiveStatusBadge, LiveTranscriptPane, MeetingLiveProvider } from './meeting-live';
...
export default async function MeetingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { meeting, segments, summary, actionItems } = detail;
  const decisions = (summary?.keyDecisions ?? []) as { text: string }[];

  // The server-rendered final transcript (authoritative once the meeting is finished). While the
  // meeting is live, LiveTranscriptPane streams in-progress segments and shows this only at the end.
  const finalTranscript =
    segments.length > 0 ? (
      ...
      <p className="rounded-[var(--radius-card)] border border-dashed border-[var(--border)] p-6 text-sm text-[var(--muted-foreground)]">
        {['complete', 'failed_processing', 'failed_recording', 'failed_join'].includes(
          meeting.status,
        )
  ...
  return (
    <MeetingLiveProvider meetingId={meeting.id} initialStatus={meeting.status}>
      <div className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
          <section className="lg:col-span-3">
            ...
            <LiveTranscriptPane fallback={finalTranscript} />
          </section>
          ...
          <aside className="flex flex-col gap-6 lg:col-span-2">
            ...
            <p className="text-xs text-[var(--muted-foreground)]">
              Audio playback isn’t available yet — recordings live in the engine’s storage. The
              transcript above is the system of record.
            </p>
          </aside>
        </div>
      </div>
